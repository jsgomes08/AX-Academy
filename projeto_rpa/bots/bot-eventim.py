...
def encontrar_shows(page):
    ...
    page.wait_for_selector('[data-qa="event-listing-item"]') # Onde os shows estão localizados na página
        ...
        # lista de ingressos
            ...
            for j in range(items.count()):
                ...
                try:
                    ...
                    print(f"  - {nome} | {preco} | {'DISPONÍVEL' if disponivel else 'INDISPONÍVEL'}")
...
def salvar_csv(evento, ingressos):
    ...
    with open(caminho, mode="w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["setor", "nome", "preco", "disponivel"])
        writer.writeheader()
...
def main():
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=False)  # 👀 abre o navegador
        ...
        # imprimir dados do primeiro
        primeiro = eventos[0]

        print("\n=== ACESSANDO SHOW ===")
        print(primeiro)

        page.goto(primeiro["link"])
